Scheduler is a web-based application that helps to manage either personal or group projects. Plan, work and get organized.

Team: 6460XYH
Team members: Zhao Huang, Yanqun Xu, Lu Yu

Files submit: 
- Paper: pdf 
- Source code(zip): including all source files
- Demo video link: https://www.youtube.com/watch?v=M8TgJfVNQls&feature=youtu.be
- Sheduler application: public URL: https://scheduler6460edu.herokuapp.com/

ps: inital login takes a little bit longer time 


If you want to run the appliation locally, you need to do the following steps:

Software needed: install intelliJ IDEA, NodeJS, npm
- install intelliJ: https://www.jetbrains.com/help/idea/installation-guide.html?section=Windows
- install NodeJS: https://nodejs.org/en/download/
- install npm: https://www.npmjs.com/package/download-npm-package

Steps for initiate backend and front end server:


1. Initiate backend server:
run cmd in root/scheduler-SpringBoot/target directory: 
$ java -jar scheduler-0.0.1-SNAPSHOT.jar
 
2. Initiate backend server: 
run cmd in Scheduler-React directory: (for the first time, run npm install first, otherwise, skip it to npm start directly
$ npm install
$ npm start

3. use chrome to open http://localhost:3000/

